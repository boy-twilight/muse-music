          ...
          <template #reference>
            <div>
              <input
                ...
                @compositionstart="chineseStart"
                @compositionend="chineseEnd"
                type="text"
...
const getSuggestData = async() => {
  try {
    ...
    const response: any = await getSuggest(search.value, signal);
    controller = null;
    signal = null;
    const {
...
// 获取搜索建议
const getSearchSuggest = throttle(() => {
